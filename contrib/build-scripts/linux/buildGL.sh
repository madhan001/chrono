...
GL_INSTALL_DIR="$HOME/Documents/chrono/libraries/gl"
...
rm -rf build_glm
mkdir build_glm
cmake -G "${BUILD_SYSTEM}" -B build_glm -S ${GLM_SOURCE_DIR} \
...
rm -rf build_glew
mkdir build_glew
cmake -G "${BUILD_SYSTEM}" -B build_glew -S ${GLEW_SOURCE_DIR}/build/cmake \
...
rm -rf build_glfw
mkdir build_glfw
cmake -G "${BUILD_SYSTEM}" -B build_glfw -S ${GLFW_SOURCE_DIR} \
      -DBUILD_SHARED_LIBS:BOOL=${BUILDSHARED} \
      -DCMAKE_BUILD_TYPE=Release \
      -DCMAKE_DEBUG_POSTFIX=_d
